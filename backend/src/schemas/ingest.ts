...
export const IngestEventTypeSchema = z.object({
  ...
  code: data.code ?? data.name.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, ''),
  name: data.name,
  description: data.description ?? null,
  baseRate: data.base_rate ?? data.base_hourly_rate ?? data.hourly_rate ?? data.rate ?? 35.0,
  riskMultiplier: data.risk_multiplier ?? data.risk_weight ?? data.risk_factor ?? data.multiplier ?? 1.0,
}));
...
export const IngestLocationSchema = z.object({
  ...
  rateModifier: data.rate_modifier ?? data.base_multiplier ?? data.location_multiplier ?? data.modifier ?? 1.0,
}));
...
export const IngestTrainingDataSchema = z.object({
  event_type: z.string(),
  ...
  return {
    ...
    cloud: typeof data.cloud === 'string'
      ? (data.cloud === 'AWS' ? 1 : data.cloud === 'Azure' ? 2 : data.cloud === 'GCP' ? 3 : 0)
      : (data.cloud ?? 0),
  };
});
